...
api.post("/login", async ({ body }, res) => {
  try {
    // Get user input
    const { email, password, enterprise, role } = body;
    ...
    if (user && (await bcrypt.compare(password, user.password))) {
      ...
      const newUser = await getUserFinded(user)
      ...
      // user
      return res.status(200).json({ error: false, data: user, message: `Connexion réussi, bienvenu chez : ${enterprise.name}` });
    } else {
